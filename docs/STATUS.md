> ※ 作業時に参照する必須規則・判定基準・EVIDENCE一覧は [RULES.md](RULES.md) を参照すること。
...
## Gemini 3.6 Flash と 3.1 Pro を同じ課題で並べた（2026-07-31 実測）

同一リポジトリ、同一プロンプト、`quote:` 必須。

**コード監査（同じ5観点）**: 両者とも `NO FINDINGS`。何度も監査して直した後なので、
判別材料にならなかった。
...
| モデル | 指摘 | 引用実在 | 新規に本物 |
|---|---|---|---|
| gemini-3.1-pro-high | 5 | 5/5 | 4 |
| gemini-3.6-flash-high | 6 | 6/6 | 3 |

**Flash が Pro の見落とし3件を当てた。**
タイムライン画面の健全性パネル（同じ形の欠陥の3件目。Pro は見た目とモードの2画面しか出さなかった）、
残っていた `color-mix` の文字色、セットアップ画面の窓レスキューの置き場所。

引用の実在率は両者 100%。**`quote:` を必須にすると、モデルの世代やティアに関係なく捏造が消える。**

結論として、**この用途では Flash が Pro に劣らない。** むしろ Pro が既に直した2画面を
繰り返し報告したのに対し、Flash は3画面目を見つけた。
速く安い側を主戦力にしてよい。ただし1回の実行で全部は出ないので、
**同じ課題を別モデルで2回当てるほうが、1回を高いモデルで回すより効いた。**

## VPN 一時接続は出荷したが、接続経路は実機で走っていない（2026-07-31）

この機に VPN が1件も登録されていないため、`registered_count=0` で
`measured=false reason=no_registered_vpn no_change=true` としか出せない。

**接続と切断の経路は一度も実行されていない。** 設計は確認した
（資格情報フィールドは全てゼロのまま `RasDialW` を呼ぶ、接続名はログに出さない、
所有はプロセス内に持ち再起動後は推測せず案内へ倒す、`Session` 種別で自動適用の対象外）。
だが**設計を読んだことは、動いたことの証明ではない。**

VPN が登録された機で測るまで、この Action は「未実証」として扱う。

## ディスプレイ着脱の修復は、動きは証明できたが着脱は測れない（2026-08-01 実測）

4回目調査の1位。`display_rescue` は**窓を一切動かさない**。
保存済み配置との差を読んで知らせるまでが自動で、戻すのは既存の Action 経路。

自分の窓だけで往復させた:

```
before=(420,260 520x360) collapsed=(160,160 520x360)
restored=(420,260 520x360) rolled_back=(160,160 520x360)
measured=false reason=single_or_cloned_monitor_topology_detach_not_measured
```

**移動と復元と巻き戻しは証明できた。着脱そのものは、この機が1画面なので測れない。**
両方を同じ行に出して、片方を他方の証明に使わないようにしてある。

2画面の機で測るまで、着脱検知は「未実証」として扱う。
`temporary_vpn` と同じ扱い（登録済み VPN が無く、接続経路が走っていない）。

## 未実証だった2件を、測れるところまで詰めた（2026-08-01 実測）

「1画面だから測れない」「VPN が無いから測れない」で止めていた2件。
**測れない部分と、測れるのに測っていなかった部分を分けた。**

### ディスプレイ着脱

`WM_DISPLAYCHANGE` が実際に飛ぶかは1画面機では測れない。
だが**構成が変わったかの判定そのものは、合成データで測れる。** 測っていなかっただけ。

2画面→1画面（外した）、1画面→2画面（挿した）、同数だが解像度違い、同数だが座標違い、
**同一構成で「違う」と言わないこと**、どの作業領域にも入らない窓の扱い——6件を固定した。

`compare_topology` を「常に Ok」に潰したら3件が落ちた。計器は効いている。

### VPN の接続経路

到達しない試験用アドレス（`198.51.100.1`）の接続先を1つ作り、**失敗経路を初めて通した。**

```
vpn_unreachable_failure registered_before=0 registered_with_test=1
                        connect_failed=true active_handles_left=false deleted_cleanly=true
```

資格情報は一切設定していない。接続は失敗し、ハンドルは残らず、接続先は消えた。
テスト後に電話帳を直接読んで、残存 0 件を確認済み。
