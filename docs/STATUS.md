...
- `Command::spawn()` が標準入出力を継承していた。起動したアプリが Totonoe のパイプを掴んだままになり、
...
**再実装せず案内に徹した。** Totonoe自身がキーフックや常駐フックを入れるのは禁止事項（injection）に触れる。
...
Action は64件から67件、ライブラリテストは252件（通常225成功、明示実機等27件）になった。
...
効果の証明にせず、Totonoeは読み取り可能な事実だけを表示し、変更はWindows自身へ委ねる。
