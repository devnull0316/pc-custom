...
- 標準権限 UI/コアと、次Taskの短命な昇格helperに備えたdeny-all IPC契約・攻撃spike
- ローカル JSON の安全なプロファイル入出力。任意コードは不可

### P1: 初期 Action

14 Action を設計対象とする。CC監査P3で格上げされたExplorer/theme/clock/transparencyの標準HKCU setterは、安全・自動化可として実装し、対象buildの実機smokeと非破壊broadcastを出荷ゲートにする。taskbar検索、Widgets、Game Mode、DND/Focusは格上げせず、書込契約が確認できるまで「検出・説明・手動案内」のみとする。
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `explorer.show_extensions` / ファイルの拡張子を常に表示する | CC監査P3承認済みのHKCU `Explorer\Advanced\HideFileExt`限定setter（`0=表示`） | キー有無、値有無、型、元の生データ、適用type/bytes、レジストリview | applied値一致時だけ元type/rawを復元。元々無ければ値を削除し、`SHChangeNotify(SHCNE_ASSOCCHANGED, ...)`で通知 | 安全。対象buildの実機smokeが出荷条件 | 不要 | OS不要。Explorer強制再起動なし | 中。Shell更新時に再smoke | raw registry状態を検出し、画面反映は別結果で表示 |
| `explorer.show_hidden` / 隠しファイルを表示する | CC監査P3承認済みのHKCU `Explorer\Advanced\Hidden`限定setter（`1=表示`）。保護されたOSファイル表示は対象外 | 同上 | 同上 | 安全。誤操作しやすいファイルが見えることは説明 | 不要 | OS不要。Explorer強制再起動なし | 中 | raw registry状態を検出し、画面反映は別結果で表示 |
...
| `taskbar.clock_seconds` / タスクバー時計に秒を表示する | CC監査P3承認済みのHKCU `Explorer\Advanced\ShowSecondsInSystemClock`限定setter（`1=表示`） | キー/値有無、type、元raw bytes、適用bytes、view | applied値一致時だけ元状態へ復元し非破壊broadcast。元々無ければ削除 | 安全。実機smokeが出荷条件 | 不要 | OS不要。Explorer強制再起動なし | 中〜高。Shell更新時に再smoke | raw registry状態は可。今回Task 2の実装対象外 |
