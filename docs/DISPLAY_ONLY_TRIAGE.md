...
| `taskbar.button_grouping` | あり（高分解能ピクセル観測実測済み） | `taskbar_button_grouping_write_changes_the_taskbar` にて実測。`taskbar_pixel_stats()` は透明度用に維持したまま、高精度な新規計器 `taskbar_fine_pixel_stats()`（色境界数・中央40%分散・輝度プロファイル）を新設。既知の変化（メモ帳2個起動）による校正試行（`known_change_delta`）を行った上で `TaskbarGlomLevel` レジストリ変更＋`restart_shell()` 実行下のタスクバーピクセル変化・完全復元および終了時 `Shell_TrayWnd` 存在確認を検証 | 保留 |
...
10. **`taskbar.button_grouping`**
    ...
    - **証拠内容**: 従来の `taskbar_pixel_stats()`（画面全体粗サンプリング）に加えて高感度な新規計器 `taskbar_fine_pixel_stats()`（色境界数・中央40%輝度分散・プロファイル比較）を追加。`taskbar_pixel_stats()` は既存動作維持のため残し、既知の変化（メモ帳0個 vs 2個起動）で 3 段階の観測方式（`edge_count`, `center_variance`, `profile_pattern`）による感度/閾値校正（`known_change_delta`）を行うプローブへ更新した。`TaskbarGlomLevel` レジストリ変更＋`restart_shell()` 実行によるタスクバーピクセル変化・完全復元、および終了時のタスクバー（`Shell_TrayWnd`）存在・可視性を実測検証。
