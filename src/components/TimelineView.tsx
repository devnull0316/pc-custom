  ...
  return (
    <div className="view timeline-view">
      ...
      {dataMode === "loading" ? (
        ...
        <div className="empty-state"><span className="empty-state__icon"><Icon name="timeline" size={28} /></span><h2>{dataMode === "catalog" ? "履歴へ接続できません" : "まだ変更はありません"}</h2><p>{dataMode === "catalog" ? "静的カタログは閲覧できますが、実際の変更履歴は安全コアへの接続が必要です。" : "項目を適用すると、変更前と変更後、そして確認の結果がここに残ります。"}</p><button className="secondary-button" onClick={onOpenActions} type="button">変更できる項目を見る</button></div>
      ) : (
